...
class TestPredictionEndpoints:
    ...
    def test_predict_json_missing_model(self, client, monkeypatch):
        ...
        response = client.post(
            "/api/predict", json={"features": {"orbital_period": 3.5, "transit_depth": 0.02}}
        )
...
"""
        ...
        response = client.post(
            "/api/predict/batch", files={"file": ("test.csv", csv_content, "text/csv")}
        )
    ...
    def test_predict_batch_invalid_file_type(self, client, mock_model_file, monkeypatch):
        ...
        response = client.post(
            "/api/predict/batch", files={"file": ("test.txt", "not a csv", "text/plain")}
        )
