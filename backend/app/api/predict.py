...
def validate_and_predict(feature_dict: dict, loader: BaseModelLoader) -> dict:
    """
    ...
    # Create response
    return create_prediction_response(predictions[0], probabilities[0], feature_importances, loader)
...
async def predict_exoplanet(request: PredictionRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid features: {str(e)}"
        )
    except Exception as e:
...
async def predict_batch(file: UploadFile = File(...)):
    ...
    try:
            ...
            try:
                ...
                for key, value in row.items():
                    try:
                        ...
                        logger.warning(
                            f"Row {row_idx}: Could not convert '{key}' value '{value}' to float"
                        )
                        continue
                ...
                predictions.append(BatchPredictionItem(row_index=row_idx, **result))
                rows_processed += 1
        ...
        return BatchPredictionResponse(
            message=message, rows_processed=rows_processed, predictions=predictions
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"CSV processing failed: {str(e)}"
        )
