...
class TestPreprocessFeatures:
    ...
    def test_flux_feature_extraction(self):
        ...
        X_without_flux, _ = preprocess_features(
            df, label_column="label", extract_flux_features=False
        )

        # With flux features should have more columns
        assert (
            X_with_flux.shape[1] > X_without_flux.shape[1]
    ...
    def test_custom_feature_columns(self):
        ...
        X, y = preprocess_features(
            df, label_column="label", extract_flux_features=False, feature_columns=custom_features
        )
