...
def _explain_with_shap(
    model: Any, X_sample: np.ndarray, feature_names: List[str], top_k: int
) -> Dict[str, Any]:
    """
    ...
    for idx in top_indices:
        top_features.append({"name": feature_names[idx], "impact": float(shap_values_signed[idx])})

    # Include all feature contributions
    all_features = {
        feature_names[i]: float(shap_values_signed[i]) for i in range(len(feature_names))
    }

    return {"top_features": top_features, "all_features": all_features}
...
def _explain_with_permutation(
    ...
    perm_importance = permutation_importance(
        model, X_sample, y_dummy, n_repeats=10, random_state=42, n_jobs=-1
    )
    ...
    top_features = []
    for idx in top_indices:
        top_features.append({"name": feature_names[idx], "impact": float(importances[idx])})

    all_features = {feature_names[i]: float(importances[i]) for i in range(len(feature_names))}

    return {"top_features": top_features, "all_features": all_features}


def _explain_with_feature_importance(
    model: Any, feature_names: List[str], top_k: int
) -> Dict[str, Any]:
    """
...
def _explain_with_uniform(feature_names: List[str], top_k: int) -> Dict[str, Any]:
    """
    ...
    top_features = [{"name": name, "impact": uniform_weight} for name in feature_names[:top_k]]
...
def explain_batch_predictions(
    ...
    for i in range(len(X_batch)):
        ...
        explanation = explain_prediction(model, X_sample, feature_names, method, top_k)
...
def get_global_feature_importance(
    model: Any, feature_names: Optional[List[str]] = None, top_k: int = 10
) -> Dict[str, Any]:
    """
    ...
    for idx in sorted_indices[:top_k]:
        top_features.append({"name": feature_names[idx], "importance": float(importances[idx])})

    return {"top_features": top_features, "method_used": "global_feature_importances"}
