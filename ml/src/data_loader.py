...
def load_exoplanet_csv(path: str) -> pd.DataFrame:
    ...
    for col, default_value in defaults.items():
        if col in df.columns:
            ...
            if missing_count > 0:
                ...
                logger.info(
                    f"Filled {missing_count} missing values in '{col}' with {default_value:.2f}"
                )
    ...
    if rows_dropped > 0:
        logger.warning(
            f"Dropped {rows_dropped} rows ({rows_dropped/initial_rows*100:.2f}%) with remaining missing values"
        )
...
def load_and_prepare_data(
    filepath: str, mission: str = "kepler", label_column: str = "LABEL"
) -> Tuple[pd.DataFrame, pd.Series]:
    """
    ...
    Returns:
        Tuple of (features, labels)
    """
    loaders = {"kepler": load_kepler_data, "k2": load_k2_data, "tess": load_tess_data}
