...
def preprocess_features(
    ...
    if flux_features_df is not None:
        ...
        logger.info(
            f"Combined features: {X_numeric.shape[1]} base + {flux_features_df.shape[1]} flux = {X_combined.shape[1]} total"
        )
    else:
...
def _extract_flux_features(flux_series: pd.Series) -> pd.DataFrame:
    ...
    logger.info(
        f"Extracted {len(features)} time-series features from {len(flux_series)} flux observations"
    )
...
def normalize_features(
    X: pd.DataFrame, method: str = "standard", scaler: Optional[object] = None
...
def remove_outliers(
    X: pd.DataFrame, y: pd.Series, n_std: float = 3.0
...
def balance_dataset(
    X: pd.DataFrame, y: pd.Series, method: str = "undersample"
) -> Tuple[pd.DataFrame, pd.Series]:
    """
